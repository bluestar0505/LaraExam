        ...
        <ul class="page-sidebar-menu"
            ...
            @foreach($menus as $menu)
                @if($menu->menu_type != 2 && is_null($menu->parent_id))
                    ...
                    @if((\App\User::isAdmin() && Auth::user()->role->canAccessMenu($menu)) || $menu->name =='suggestions' || $menu->name =='Notify' || $menu->name =='test')
                        <li @if(isset(explode('/',Request::path())[1]) && explode('/',Request::path())[1] == strtolower($menu->name)) class="active" @endif>
                            <a href="{{ route(config('quickadmin.route').'.'.strtolower($menu->name).'.index') }}">
                                ...
                                @if($menu->title == 'Notify')
                                    <span class="title">{{ $menu->title }}
                                        ( new
                                        {{ DB::table('notification_users')
                                            ->where('IsSeen', '0')->where('UserId', Auth::user()->id)
                                            ->count() }}
                                        )
                                    </span>
                                @else
            ...
            @endforeach
                <li>
                    <a href="{{ route(strtolower(config('quickadmin.route').'.purchases')) }}">
                        <i class="fa fa-list"></i>
